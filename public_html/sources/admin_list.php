...
function deljob(){
	...
	if($_POST['param'] > 0){
		$chenge->delete(false,"admins","admin_id=" . $_POST['param']);
	}
}
...
<a href="admin_detail.php?mid={$rows[$key]['admin_id']}{$urlparam}">{$rows[$key]['admin_name']}</a>
</td>
<td width="15%" class="center{$nobottom}">
<input type="button" value="削除確認" onClick="javascript:del_func_form({$rows[$key]['admin_id']},'{$javamsg}')" />
</td>
</tr>
END_BLOCK;
		$retstr .= $str;
