...
function deljob(){
	...
	if($_POST['param'] > 0){
		$chenge->delete(false,"orders","order_id=" . $_POST['param']);
	}
}
...
<a href="orders_detail.php?mid={$rows[$key]['order_id']}{$urlparam}">{$rows[$key]['item_name']}</a>
</td>
<td width="15%" class="center{$nobottom}">
<input type="button" value="削除確認" onClick="javascript:del_func_form({$rows[$key]['order_id']},'{$javamsg}')" />
</td>
</tr>
END_BLOCK;
		$retstr .= $str;
...
<p><a href="orders_detail.php">新規</a></p>
