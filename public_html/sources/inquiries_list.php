...
function deljob(){
	...
	if($_POST['param'] > 0){
		$chenge->delete(false,"inquirys","inquiry_id=" . $_POST['param']);
	}
}
...
function echo_inquiries_list(){
	...
	if(count($rows) > 0){
		...
		$javamsg =  '【' . $rows[$key]['inquiry_title'] .'】';
		$nobottom = '';
...
<a href="inquiries_detail.php?mid={$rows[$key]['inquiry_id']}{$urlparam}">{$rows[$key]['inquiry_title']}</a>
</td>
<td width="15%" class="center{$nobottom}">
<input type="button" value="削除確認" onClick="javascript:del_func_form({$rows[$key]['inquiry_id']},'{$javamsg}')" />
</td>
</tr>
END_BLOCK;
		$retstr .= $str;
...
<p><a href="inquiries_detail.php">新規</a></p>
