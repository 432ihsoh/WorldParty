...
function deljob(){
	...
	if($_POST['param'] > 0){
		$chenge->delete(false,"items","item_id=" . $_POST['param']);
	}
}
...
<a href="items_detail.php?mid={$rows[$key]['item_id']}{$urlparam}">{$rows[$key]['item_name']}</a>
</td>
<td width="15%" class="center{$nobottom}">
<input type="button" value="削除確認" onClick="javascript:del_func_form({$rows[$key]['item_id']},'{$javamsg}')" />
</td>
</tr>
END_BLOCK;
		$retstr .= $str;
...
<p><a href="items_detail.php">新規</a></p>
